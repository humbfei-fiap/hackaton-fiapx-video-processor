...
def test_register_user_success():
    ...
    # Act
    result = use_case.execute("testuser", "password123")
...
def test_register_user_already_exists():
    # Arrange
    mock_repo = MagicMock()
    mock_repo.get_by_username.return_value = User(username="existing", password="xxx")
    
    use_case = RegisterUserUseCase(mock_repo, MagicMock())
    
    # Act & Assert
    with pytest.raises(ValueError) as excinfo:
        use_case.execute("existing", "password123")
